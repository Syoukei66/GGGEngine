...
void AssetManager::ReserveLoad(const IAssetContainer* resource)
{
  this->load_reserve_.insert(const_cast<IAssetContainer*>(resource));
}

//loaded_resources_release … 既にロード済みのAssetを解放する場合true
void AssetManager::PreRealize(IAssetLoadingListener* listener, bool loaded_resources_release)
{
  if (listener)
  {
...
void AssetManager::Realize(IAssetLoadingListener* listener)
{
  if (listener)
  {
