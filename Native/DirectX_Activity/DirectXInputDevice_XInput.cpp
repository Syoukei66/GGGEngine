...
void DirectXInputDevice_XInput::InputProcess(T_UINT8 handler, EngineInputState* state)
{
  ...
  const T_FLOAT thumbL_x = (xstate.Gamepad.sThumbLX + 32768.0f) / 65535.0f;
  const T_FLOAT thumbL_y = (xstate.Gamepad.sThumbLY + 32768.0f) / 65535.0f;
  ...
  const T_FLOAT thumbR_x = (xstate.Gamepad.sThumbRX + 32768.0f) / 65535.0f;
  const T_FLOAT thumbR_y = (xstate.Gamepad.sThumbRY + 32768.0f) / 65535.0f;
