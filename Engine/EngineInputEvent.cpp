...
void EngineInputState::Prepare()
{
  ...
  for (std::map<T_UINT8, AnalogInputState>::iterator itr = this->analog_input_.begin(), v_end = this->analog_input_.end(); itr != v_end; ++itr)
  {
    (*itr).second.Prepare();
  }
  //for (std::map<T_UINT8, DigitalInputState>::iterator itr = this->digital_input_.begin(); itr != this->digital_input_.end(); ++itr)
  for (std::map<T_UINT8, DigitalInputState>::iterator itr = this->digital_input_.begin(), v_end = this->digital_input_.end(); itr != v_end; ++itr)
  {
    (*itr).second.Prepare();
  }
}
