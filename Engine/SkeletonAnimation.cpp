...
void SkeletonAnimation::NativeDraw(GameObject2DRenderState* state)
{
  ...
  for (int i = 0, n = skeleton_->slotsCount; i < n; ++i)
  {
    ...
    NativeMethod::Graphics().Graphics_DrawSprite(state, INativeProcess_Graphics::PRIMITIVE_TRIANGLESTRIP, vertexes, attachment_vertexes->size_, attachment_vertexes->texture_);
