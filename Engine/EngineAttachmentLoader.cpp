...
void EngineAttachmentLoader_configureAttachment(spAttachmentLoader* loader, spAttachment* attachment) {
  ...
  AttachmentVertexes* attachment_vertexes = new AttachmentVertexes((LP_TEXTURE)region->page->rendererObject, 4);
  SpriteVertex* vertexes = attachment_vertexes->vertexes_;
  for (int i = 0, ii = 0; i < 4; ++i, ii += 2) {
    vertexes[i].uv.x = region_attachment->uvs[ii];
    vertexes[i].uv.y = region_attachment->uvs[ii + 1];
  }
